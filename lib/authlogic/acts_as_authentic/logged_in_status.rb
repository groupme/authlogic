      ...
      module Methods
        def self.included(klass)
          ...
          klass.class_eval do
            ...
            scope_method = respond_to?(:scope) ? :scope : :name_scoped
            send(scope_method, :logged_in, lambda { {:conditions => ["last_request_at > ?", logged_in_timeout.seconds.ago]} })
            send(scope_method, :logged_out, lambda { {:conditions => ["last_request_at is NULL or last_request_at <= ?", logged_in_timeout.seconds.ago]} })
          end
        end
